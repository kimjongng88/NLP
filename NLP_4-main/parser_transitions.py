...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            head = self.stack[-1] if transition == "LA" else self.stack[-2]
            dependent = self.stack.pop(-2) if transition == "LA" else self.stack.pop(-1)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for partialParse, transition in zip(minibatch, transitions):
            partialParse.parse_step(transition=transition)
            if len(partialParse.buffer) == 0 and len(partialParse.stack) ==1:
                unfinished_parses.remove(partialParse)
